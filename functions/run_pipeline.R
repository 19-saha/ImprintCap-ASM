...
# ── Fixed folder layout (user never edits these) ─────────────────────────────
...
# ── Auto-discover all samples from input/ ────────────────────────────────────
...
run_sample <- function(sample_id) {
  ...
  cpg_snp_xlsx <- file.path(results_dir,
                              paste0("cpg_snps_", sample_type, "_",
                                     sample_id, ".xlsx"))
  bed_file     <- file.path(results_dir,
                              paste0("cpg_snps_", sample_type, "_",
                                     sample_id, ".bed"))
  wide_bam     <- file.path(bam_asm_dir,
                              paste0(sample_type, "_", sample_id,
                                     "_all_wide.bam"))

  cat("Processing:", sample_id, "[", sample_type, "]\n")
  ...
  cat("  ✓ Done:", sample_id, "[", sample_type, "]\n\n")
}

# ── Batch run all discovered samples ─────────────────────────────────────────
...
cat("DONE —", sum(!sapply(results, is.null)), "/",
    length(sample_ids), "succeeded\n")
