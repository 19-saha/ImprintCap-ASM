ASM <- function(cpg_snp_file, sam_file, filter_cpgs_file, output_file = NULL) {
  library(data.table)
  library(readxl)
  library(writexl)
  library(ggplot2)
  ...
  make_lineplot_pdf <- function(snp_cpg_dt, pdf_file) {
    ...
    plot_data[, Allele_Type := ifelse(Allele_Type == "REF_f", "REF Allele", "ALT Allele")]
  ...
  cpg_ref_raw[, mean_methylation := round(rowMeans(.SD, na.rm = TRUE), 2), .SDcols = sample_cols]
  cpg_ref_raw[, sd_methylation   := round(apply(.SD, 1, sd, na.rm = TRUE), 2), .SDcols = sample_cols]
  cpg_ref_raw[, `:=`(
  ...
  cpg_ref_raw[, Category := factor(Category, levels = c("MSDvar", "SDvar", "Mvar", "LOWvar"))]
  ...
  sam_dt <- data.table(
    ...
    md_tag     = sapply(bam_scan$tag$MD, function(x) if (is.null(x)) NA_character_ else as.character(x)),
  ...
  cpg_counts <- final_results[
    allele_type %in% c("REF", "ALT"),
    .(n = .N),
    by = .(cpg_pos, snp_pos, allele_type)
  ]
  cpg_counts_wide <- dcast(cpg_counts, cpg_pos + snp_pos ~ allele_type,
                           value.var = "n", fill = 0L)
  valid_cpg_snp <- cpg_counts_wide[
    get("REF") >= 20L & get("ALT") >= 20L &
      get("REF") / get("ALT") <= 4 & get("ALT") / get("REF") <= 4,
    .(cpg_pos, snp_pos)
  ]
  ...
  final_results <- merge(final_results, snp_groups, by = c("DMR", "snp_pos"), all.x = TRUE)
  ...
  meth_sum_file <- paste0("meth_summary_", sample_id, ".xlsx")
  pdf_file      <- paste0("dmr_plots_",    sample_id, ".pdf")
